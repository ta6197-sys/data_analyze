...
| 배점 | 항목 | 체크 |
|---|---|---|
| 35점 | 환경구성 + 비동기 수집 | venv 활성화 상태에서 requirements.txt로 설치, 3개 API 모두 gather()로 동시 수집·응답 확인 |
| 45점 | 스키마 검증 + 저장 비교 | Pydantic v2 모델, 타입 오류 시 예외 처리, CSV·Parquet 모두 저장 + 성능 측정 결과 출력 |
| 10점 | 테스트·커밋 | pytest 1건 이상 통과, ruff 오류 없음, git 커밋 이력 존재 |
| 10점 | 완성도 | **주석 누락 시 감점** — 각 함수/블록에 목적 주석 필수 |
